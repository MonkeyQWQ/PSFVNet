...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'dataset2'))
from simple_video_dataset import get_simple_video_dataloader
...
def setup_logging(log_dir):
    ...
    log_file = os.path.join(log_dir, f'train_v3_optimized_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
...
def train():
    ...
    logging.info("UltraVideoSnowNet V3-Optimized 训练开始")

    os.makedirs(checkpoint_dir, exist_ok=True)

    print("📥 加载训练数据...")
    train_loader = get_simple_video_dataloader(
        data_root=data_root,
        split='train',
    ...
    print("📥 加载验证数据...")
    test_loader = get_simple_video_dataloader(
        data_root=data_root,
        split='val',
        batch_size=1,
        window_size=num_frames,
    ...
    print("✅ V3-Optimized模型创建成功")

    total_params = sum(p.numel() for p in model.parameters())
    print(f"📊 模型参数量: {total_params / 1e6:.2f}M")
    print(f"📊 预期: ~5.2M (相比V3的6.0M，减少13%)\n")
